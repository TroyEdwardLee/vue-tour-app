      ...
      <div class="list">
        <div v-for="(item, index) in list" :key="index">
          <div v-if="item.type ==='department' && item.children.length > 0">
            <label>
              <i
                :class="item.open ? 'el-icon-caret-bottom' : 'el-icon-caret-right'"
                @click.stop.prevent="openNext(item)"
              ></i>
              <input
                :data-name="item.name"
            ...
            <div v-show="item.open" v-for="(childItem, index) in item.children" :key="index">
              <div
                class="child"
                v-if="childItem.type ==='department' && childItem.children.length > 0"
              >
                <label>
                  <i
                    :class="childItem.open ? 'el-icon-caret-bottom' : 'el-icon-caret-right'"
                    @click.stop.prevent="openNext(childItem)"
                  ></i>
                  <input
                ...
                <div v-show="childItem.open" v-for="(thirdItem, index) in childItem.children" :key="index">
                  <div
                    class="third"
                    v-if="thirdItem.type ==='department' && thirdItem.children.length > 0"
                  >
                    <label>
                      <i class="el-icon-caret-right"></i>
                      <input
                        :data-name="thirdItem.name"
                        :data-level="thirdItem.level"
                        type="checkbox"
                        @change="selectItem"
                  ...
                  <div
                    class="humanItem"
                    v-else-if="thirdItem.type === 'human' && thirdItem.children.length === 0"
                  >
                    <label>
                      <input
                        :data-name="thirdItem.name"
              ...
              <div
                class="humanItem"
                v-else-if="childItem.type === 'human' && childItem.children.length === 0"
              >
                <label>
                  <input
                    :data-name="childItem.name"
          ...
          <div
            class="humanItem single"
            v-else-if="item.type === 'human' && item.children.length === 0"
          >
            <label>
              <input
                :data-name="item.name"
      ...
      <div class="list">
        <div v-for="(item, index) in list" :key="index">
          <div v-if="item.type ==='department' && item.children.length > 0">
            ...
            <div v-show="!item.checked" v-for="(childItem, index) in item.children" :key="index">
              <div
                class="child"
                v-if="childItem.type ==='department' && childItem.children.length > 0"
              >
                ...
                <div v-show="!childItem.checked" v-for="(thirdItem, index) in childItem.children" :key="index">
                  <div
                    class="third"
                    v-if="thirdItem.type ==='department' && thirdItem.children.length > 0"
                  >
      ...
      if (organizationData[i].children.length > 0) {
        for (let j = 0; j < organizationData[i].children.length; j++) {
          ...
          if (organizationData[i].children[j].children.length > 0) {
            for (
              let k = 0;
              k < organizationData[i].children[j].children.length;
              k++
            ) {
      ...
      handler: function (val, oldVal) {
        ...
        for (let i = 0; i < val.length; i++) {
          ...
          if (val[i].type === 'department') {
            if (
              val[i].children.every(function (item) {
                return item.checked
              })
            ) {
        ...
        if (this.list[i].checked) {
          ...
          for (let j = 0; j < this.list[i].children.length; j++) {
            if (this.list[i].children[j].checked) {
              ...
              for (
                let k = 0;
                k < this.list[i].children[j].children.length;
                k++
              ) {
            ...
            if (this.list[i].children.length > 0) {
              for (let j = 0; j < this.list[i].children.length; j++) {
                ...
                for (
                  ...
                  this.$set(
                    _this.list[i].children[j].children[k],
                    'checked',
                    Status
                  )
                }
              ...
              if (this.list[i].children[j].children.length > 0) {
                for (
                  ...
                  if (this.list[i].children[j].children[k].name === words) {
                    ...
                    if (
                      this.list[i].children[j].children.every(function (item) {
                        return item.checked
                      })
                    ) {
